...
Still to build: expanded read-only SDK (`query_data`/`ask_agent`/`notify`), canvas code-edit view (frontend), inter-step shared workdir (v0 passes files by absolute path via variables).
