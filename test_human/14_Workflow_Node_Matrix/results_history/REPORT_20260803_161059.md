# Workflow Node Regression Report — 20260803_161059

- Build: `95228f7` | Base: `http://localhost:5001` | Tier <= 3 | Baseline: `results_20260802_204331.json`
- Outputs: `C:\temp\aihub_test\nodereg\20260803_161059`

## Verdict: **REGRESSIONS DETECTED** — 1 FAIL / 23 PASS / 11 SKIP / 3 XFAIL

## 🔴 REGRESSIONS vs baseline (was PASS, now broken)

| check | was | now |
|---|---|---|
| portal_node_run | PASS | **FAIL** |
...
| human_approval_approve | 2 | ✅ PASS | status=completed; decided=True (req=DF66D63C-07FA-4082-AB2A-4B92366FD07E, http=200); post-approval file=True |
...
| portal_node_run | 2 | ❌ FAIL | status=failed; portal-status=error; files=0; portal-error=Failed to establish CDP connection to browser: Failed to get session for initial target 573BB0D94C311F24704829619C7F668F \| log-tail: Executing node: portal probe (Portal) ~ Workflow execution started:  |
| file_transfer_sftp_upload | 2 | ✅ PASS | status=completed; remote-file=True (xfer_20260803_161059.txt); content-ok=True |
...
| comp_excel_export_throughput | 3 | ⚠️ XFAIL | status=completed; 40 rows in 55.8s = 0.72 rows/sec (want >= 5.0); 1000 rows would take ~23.2 min \| log-tail: Executing node: big (Database) ~ Workflow execution started: NODEREG-comp_excel_export_throughput |
