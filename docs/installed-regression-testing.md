...
| The Agent front door | `THE_AGENT_ENABLED=true` and `THE_AGENT_MODE=true` in the shipped `.env` for testing. The nav entry, `/the-agent` and the `/` redirect are all gated to Developers/Admins unless `AGENT_ALLOW_ALL_USERS=true` (the service's own gate), so Developers/Admins land in The Agent from `/` (`?classic=1` is the sticky escape) and regular users see exactly what they see today. Installer seeds `AGENT_ALLOW_ALL_USERS=false`. Upgrades keep their existing `.env`, so the 10.0.0.6 box needs both flags flipped by hand. | template parse + route/context/home-redirect tests |
