...
# ------------------------------------------------------------------ walkthroughs
# Role-scenario walkthroughs (test_human/17_Business_Role_Scenarios/*/walkthrough.json).
# Each is GENERATED by its pack's walkthrough.py, with every expected value pulled from
# that pack's oracles — so the panel can never drift from the answer key. The panel only
# reads them; progress lives beside the panel in runs/.
RUNS = os.path.join(HERE, "runs")
WALKTHROUGHS = {}     # id -> walkthrough dict


def load_walkthroughs():
    WALKTHROUGHS.clear()
    for path in SET.get("walkthroughs", []):
        try:
            with open(path, encoding="utf-8") as fh:
                wt = json.load(fh)
            wt["_path"] = path
            WALKTHROUGHS[wt["id"]] = wt
        except FileNotFoundError:
            print(f"  walkthrough not generated yet: {path}")
        except Exception as e:      # noqa: BLE001 — never let one bad file kill the panel
            print(f"  walkthrough failed to load ({path}): {type(e).__name__}: {e}")


load_walkthroughs()


def _run_path(wid, run_id):
    safe = re.sub(r"[^A-Za-z0-9_.-]", "_", f"{wid}__{run_id}")
    return os.path.join(RUNS, safe + ".json")


def load_run(wid, run_id):
    try:
        with open(_run_path(wid, run_id), encoding="utf-8") as fh:
            return json.load(fh)
    except Exception:               # noqa: BLE001 — a missing/corrupt run starts fresh
        return {"walkthrough": wid, "run_id": run_id, "steps": {},
                "started": time.time(), "meta": {}}


def save_run(wid, run_id, data):
    os.makedirs(RUNS, exist_ok=True)
    data["updated"] = time.time()
    tmp = _run_path(wid, run_id) + ".tmp"
    with open(tmp, "w", encoding="utf-8") as fh:
        json.dump(data, fh, indent=2)
    os.replace(tmp, _run_path(wid, run_id))
    return data
...
def list_runs(wid):
    ...
    for f in sorted(os.listdir(RUNS)):
        if f.startswith(re.sub(r"[^A-Za-z0-9_.-]", "_", wid) + "__") and f.endswith(".json"):
            try:
                with open(os.path.join(RUNS, f), encoding="utf-8") as fh:
                    d = json.load(fh)
                marks = d.get("steps", {})
                out.append({"run_id": d.get("run_id"), "updated": d.get("updated"),
                            "counts": {k: sum(1 for v in marks.values() if v.get("status") == k)
...
def run_report(wid, run_id):
    """Render a run as the markdown report that goes next to TEST_RUN_TEMPLATE.md."""
    wt = WALKTHROUGHS.get(wid)
    data = load_run(wid, run_id)
    marks = data.get("steps", {})
    icon = {"pass": "✅", "fail": "❌", "warn": "⚠️", "skip": "—"}

    tally = {k: sum(1 for v in marks.values() if v.get("status") == k)
             for k in ("pass", "fail", "warn", "skip")}
    blockers = []
    for b in wt["beats"]:
        for s in b["steps"]:
            m = marks.get(s["id"], {})
            if s.get("blocking") and m.get("status") in ("fail", "warn"):
                blockers.append((s["id"], s.get("title", ""), m))

    L = [f"# {wt['title']} — run report", "",
         f"**Run:** `{run_id}` · **Anchor:** `{wt.get('anchor')}` · "
         f"**Exported:** {time.strftime('%Y-%m-%d %H:%M')}", ""]
    for k, v in (data.get("meta") or {}).items():
        if v:
            L.append(f"**{k.title()}:** {v}")
    L += ["", f"**Score:** {tally['pass']} ✅ · {tally['fail']} ❌ · {tally['warn']} ⚠️ · "
              f"{tally['skip']} skipped, of {sum(len(b['steps']) for b in wt['beats'])} steps", ""]

    if blockers:
        L += ["## ⛔ Release-blocking failures", ""]
        for sid, title, m in blockers:
            L.append(f"- **{sid}** {title}" + (f" — {m.get('note')}" if m.get("note") else ""))
        L.append("")
    else:
        marked = [s for b in wt["beats"] for s in b["steps"] if marks.get(s["id"], {}).get("status")]
        L += ["## Release-blocking", "",
              ("No blocking step failed." if marked
               else "_Nothing marked yet — this run has not been scored._"), ""]

    for b in wt["beats"]:
        done = [s for s in b["steps"] if marks.get(s["id"], {}).get("status")]
        L += [f"## Beat {b['num']} — {b['title']}  ({len(done)}/{len(b['steps'])} marked)", "",
              "| Step | | Result | Note |", "|---|---|---|---|"]
        for s in b["steps"]:
            m = marks.get(s["id"], {})
            st = m.get("status")
            note = (m.get("note") or "").replace("|", r"\|")
            L.append(f"| `{s['id']}` | {'🔒' if s.get('blocking') else ''} | "
                     f"{icon.get(st, '·')} {st or 'not run'} | {note} |")
        L.append("")
    return "\n".join(L) + "\n"
...
def do_action(jid, aid):
    ...
    try:
        if kind == "spawn":
            ...
            cmd = [_py(a["python"]), a["script"], *[str(x) for x in a.get("args", [])]]
            r = subprocess.run(cmd, cwd=a.get("cwd") or HERE,
...
# ------------------------------------------------------------------ walkthrough API
@app.route("/api/walkthroughs")
def api_walkthroughs():
    return jsonify([
        {"id": w["id"], "title": w["title"], "role": w.get("role"),
         "persona": w.get("persona"), "premise": w.get("premise"), "anchor": w.get("anchor"),
         "beats": len(w["beats"]), "steps": sum(len(b["steps"]) for b in w["beats"]),
         "blocking": sum(1 for b in w["beats"] for s in b["steps"] if s.get("blocking")),
         "runs": list_runs(w["id"])}
        for w in WALKTHROUGHS.values()])


@app.route("/api/wt/<wid>")
def api_wt(wid):
    wt = WALKTHROUGHS.get(wid)
    if not wt:
        return jsonify({"error": "unknown walkthrough"}), 404
    run_id = request.args.get("run") or time.strftime("%Y-%m-%d")
    return jsonify({"wt": wt, "run": load_run(wid, run_id), "run_id": run_id,
                    "runs": list_runs(wid),
                    "actions": {k: {"name": v.get("name"), "confirm": v.get("confirm")}
                                for k, v in REG["actions"].items() if k.startswith("ar-")}})


@app.route("/api/wt/<wid>/mark", methods=["POST"])
def api_wt_mark(wid):
    if wid not in WALKTHROUGHS:
        return jsonify({"error": "unknown walkthrough"}), 404
    body = request.get_json(silent=True) or {}
    run_id = body.get("run_id") or time.strftime("%Y-%m-%d")
    data = load_run(wid, run_id)
    sid = body.get("step")
    if sid:
        entry = data["steps"].get(sid, {})
        if "status" in body:
            # Clicking the same verdict again clears it — no way to un-mark otherwise.
            entry["status"] = None if entry.get("status") == body["status"] else body["status"]
        if "note" in body:
            entry["note"] = body["note"]
        entry["ts"] = time.time()
        data["steps"][sid] = {k: v for k, v in entry.items() if v is not None or k == "note"}
        if data["steps"][sid].get("status") is None:
            data["steps"][sid].pop("status", None)
    if "meta" in body:
        data.setdefault("meta", {}).update(body["meta"])
    save_run(wid, run_id, data)
    return jsonify(data)


@app.route("/api/wt/<wid>/report")
def api_wt_report(wid):
    if wid not in WALKTHROUGHS:
        return jsonify({"error": "unknown walkthrough"}), 404
    run_id = request.args.get("run") or time.strftime("%Y-%m-%d")
    md = run_report(wid, run_id)
    if request.args.get("save"):
        wt = WALKTHROUGHS[wid]
        dest = os.path.join(os.path.dirname(wt["_path"]), f"TEST_RUN_{run_id}.md")
...
@app.route("/api/wt/<wid>/doc/<path:name>")
def api_wt_doc(wid, name):
    """Serve a scenario markdown file so 'full detail' opens without leaving the panel."""
    wt = WALKTHROUGHS.get(wid)
    if not wt:
        return Response("unknown walkthrough", status=404, mimetype="text/plain")
    full = os.path.join(os.path.dirname(wt["_path"]), name)
    if not os.path.isfile(full) or not os.path.abspath(full).startswith(
            os.path.abspath(os.path.dirname(wt["_path"]))):
        return Response(f"not found: {name}", status=404, mimetype="text/plain")
...
async function state(){
  ...
  if(!WT.length){ try { WT = await (await fetch('/api/walkthroughs')).json(); } catch(e){ WT=[]; } }
  render();
}
function renderScenarios(){
  const el = document.getElementById('scenarios');
  if(!WT.length){ el.innerHTML=''; return; }
  el.innerHTML = '<h2>Role scenarios — guided day-in-the-life walkthroughs</h2><div class="cards">' +
    WT.map(w=>{
      const last = (w.runs||[])[0];
      const c = last ? last.counts : null;
      const prog = c ? (c.pass+' ✅ '+c.warn+' ⚠️ '+c.fail+' ❌ of '+w.steps+' steps')
                     : 'not started';
      const chip = c && c.fail ? '<span class="chip notready">'+c.fail+' FAILED</span>'
                 : (c ? '<span class="chip ready">IN PROGRESS</span>'
                      : '<span class="chip unknown">NOT STARTED</span>');
      return '<div class="card"><div class="top"><div><h3>'+w.title+'</h3>'+
        '<div class="cat">'+w.beats+' beats · '+w.steps+' steps · '+w.blocking+
        ' release-blocking · book anchored '+w.anchor+'</div></div>'+chip+'</div>'+
        '<div class="tag">'+w.premise+'</div>'+
        '<div class="deps"><span>'+prog+(last?' — last run '+last.run_id:'')+'</span></div>'+
        '<div class="row">'+
        '<a class="btn small" style="text-decoration:none" href="/walkthrough/'+w.id+'">▶ Start walkthrough</a>'+
        '<button class="btn small" onclick="act(\'ar-seed\')">♻ Seed the book</button>'+
        '<button class="btn small ghost" onclick="act(\'ar-verify\')">✓ Verify vs answer key</button>'+
        '<button class="btn small ghost" onclick="act(\'ar-inspect\')">🔍 Inspect data</button>'+
        '</div></div>';
    }).join('') + '</div>';
}
...
# ------------------------------------------------------------------ walkthrough runner UI
WT_PAGE = r"""<!doctype html>
<html><head><meta charset="utf-8"><title>Walkthrough — AI Hub Demo Control Panel</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
 * { box-sizing: border-box; margin: 0; }
 body { font-family: 'Segoe UI', system-ui, sans-serif; background: #0e151d; color: #d7e1ea; }
 a { color: #6db7e8; }
 header { background: #101c28; border-bottom: 1px solid #1f3140; padding: 11px 22px;
          display: flex; align-items: center; gap: 14px; position: sticky; top: 0; z-index: 20; }
 header h1 { font-size: 16px; color: #e8f0f7; font-weight: 600; }
 header .sub { font-size: 11.5px; color: #6d8296; }
 .btn { background: #14405a; color: #cfe6f5; border: 1px solid #1f5f85; border-radius: 7px;
        padding: 6px 13px; font-size: 12.5px; cursor: pointer; font-weight: 600; }
 .btn:hover { background: #175177; }
 .btn.small { padding: 3px 9px; font-size: 11.5px; font-weight: 500; }
 .btn.ghost { background: transparent; border-color: #2a4356; color: #9db4c6; }
 .btn.danger { background: #4a1f24; border-color: #7a343c; color: #f0c9cd; }
 .right { margin-left: auto; display: flex; gap: 9px; align-items: center; font-size: 12px; }
 .wrap { display: flex; align-items: flex-start; }
 nav { width: 268px; flex: none; padding: 16px 12px 60px 18px; position: sticky; top: 49px;
       max-height: calc(100vh - 49px); overflow-y: auto; }
 nav .bt { display: block; width: 100%; text-align: left; background: #121d29;
           border: 1px solid #1f3140; border-radius: 9px; padding: 9px 11px; margin-bottom: 7px;
           cursor: pointer; color: #d7e1ea; }
 nav .bt:hover { border-color: #2f5473; }
 nav .bt.on { background: #16293a; border-color: #3d7fae; }
 nav .bt .n { font-size: 11px; color: #6d8296; }
 nav .bt .t { font-size: 13px; font-weight: 600; color: #e8f0f7; margin: 1px 0 5px; }
 nav .bar { height: 4px; background: #1b2a38; border-radius: 3px; overflow: hidden; display: flex; }
 nav .bar i { display: block; height: 100%; }
 nav .bar .p { background: #3fae6d; } nav .bar .w { background: #d9a53c; }
 nav .bar .f { background: #e05561; } nav .bar .s { background: #4a5f70; }
 main { flex: 1; min-width: 0; padding: 16px 24px 90px; max-width: 1120px; }
 .beathead { background: #121d29; border: 1px solid #1f3140; border-left: 4px solid #3d7fae;
             border-radius: 10px; padding: 14px 18px; margin-bottom: 14px; }
 .beathead h2 { font-size: 18px; color: #e8f0f7; }
 .beathead .meta { font-size: 12px; color: #6d8296; margin-top: 3px; }
 .beathead .why { font-size: 13px; color: #9db4c6; margin-top: 9px; font-style: italic; }
 .beathead .warn { background: #33270f; border: 1px solid #7a6023; color: #f0e3c0;
                   border-radius: 7px; padding: 8px 11px; font-size: 12.5px; margin-top: 10px; }
 .step { background: #121d29; border: 1px solid #1f3140; border-radius: 10px; padding: 0;
         margin-bottom: 13px; overflow: hidden; }
 .step.done { border-color: #23573a; }
 .step.bad { border-color: #7a343c; }
 .step .hd { display: flex; align-items: center; gap: 9px; padding: 11px 16px;
             background: #101a24; border-bottom: 1px solid #1f3140; }
 .step .sid { font-family: Consolas, monospace; font-size: 11.5px; color: #6db7e8; }
 .step .stt { font-size: 14px; font-weight: 600; color: #e8f0f7; }
 .step .where { font-size: 11.5px; color: #6d8296; margin-left: auto; text-align: right; }
 .badge { font-size: 10px; font-weight: 700; padding: 2px 7px; border-radius: 10px;
          text-transform: uppercase; letter-spacing: .4px; }
 .badge.block { background: #4a1f24; color: #f08a94; }
 .badge.kind { background: #1a2c3c; color: #9dc4de; }
 .body { padding: 14px 16px; }
 .instr { font-size: 13.5px; color: #cbd9e5; margin-bottom: 11px; line-height: 1.55; }
 .lbl { font-size: 10.5px; text-transform: uppercase; letter-spacing: .7px; color: #6d8296;
        margin: 0 0 5px; font-weight: 700; }
 .promptbox { position: relative; background: #0b1117; border: 1px solid #26445c;
              border-radius: 8px; padding: 12px 14px; margin-bottom: 12px; }
 .promptbox pre { font-family: 'Segoe UI', system-ui, sans-serif; font-size: 13px; color: #dbe8f3;
                  white-space: pre-wrap; line-height: 1.5; margin-right: 74px; }
 .promptbox .cp { position: absolute; top: 9px; right: 9px; }
 .inject { background: #33270f; border: 1px solid #7a6023; border-left: 4px solid #d9a53c;
           border-radius: 7px; padding: 10px 13px; margin-bottom: 12px; }
 .inject .lbl { color: #d9a53c; }
 .inject pre { font-family: Consolas, monospace; font-size: 12px; color: #f0e3c0;
               white-space: pre-wrap; line-height: 1.5; }
 .cols { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
 @media (max-width: 900px) { .cols { grid-template-columns: 1fr; } nav { display: none; } }
 .exp { background: #0f1a15; border: 1px solid #23573a; border-radius: 8px; padding: 11px 13px; }
 .exp .lbl { color: #4fc07f; }
 .fl { background: #1c1114; border: 1px solid #5c2a30; border-radius: 8px; padding: 11px 13px; }
 .fl .lbl { color: #e0808a; }
 ul.e { margin: 0; padding-left: 17px; }
 ul.e li { font-size: 12.8px; color: #c6d6e2; line-height: 1.6; margin-bottom: 4px; }
 .headline { font-size: 15px; font-weight: 700; color: #6fe0a1; margin-bottom: 8px; }
 table.x { width: 100%; border-collapse: collapse; margin: 9px 0; font-size: 12px; }
 table.x caption { text-align: left; font-size: 11px; color: #8fb4cf; padding-bottom: 5px;
                   font-weight: 700; text-transform: uppercase; letter-spacing: .5px; }
 table.x th { text-align: left; color: #6d8296; font-size: 10.5px; text-transform: uppercase;
              padding: 4px 8px; border-bottom: 1px solid #23384a; }
 table.x td { padding: 4px 8px; border-bottom: 1px solid #18242f; color: #c6d6e2;
              vertical-align: top; }
 .note { background: #16202b; border-left: 3px solid #4a6d88; border-radius: 5px;
         padding: 9px 12px; font-size: 12.5px; color: #9db4c6; margin-top: 11px; line-height: 1.55; }
 .cmd { background: #0b1117; border: 1px solid #1f3140; border-radius: 7px; padding: 9px 11px;
        margin-top: 11px; position: relative; }
 .cmd code { font-family: Consolas, monospace; font-size: 11.5px; color: #9fd0a8;
             word-break: break-all; display: block; margin-right: 150px; }
 .cmd .acts { position: absolute; top: 7px; right: 8px; display: flex; gap: 5px; }
 .verdict { display: flex; gap: 7px; align-items: center; padding: 11px 16px;
            background: #101a24; border-top: 1px solid #1f3140; flex-wrap: wrap; }
 .v { border: 1px solid #2a4356; background: transparent; color: #9db4c6; border-radius: 7px;
      padding: 5px 14px; font-size: 12.5px; cursor: pointer; font-weight: 600; }
 .v:hover { border-color: #4a7ea5; }
 .v.on.pass { background: #14402a; border-color: #3fae6d; color: #6fe0a1; }
 .v.on.warn { background: #4a3a11; border-color: #d9a53c; color: #ecc45c; }
 .v.on.fail { background: #4a1f24; border-color: #e05561; color: #f08a94; }
 .v.on.skip { background: #223140; border-color: #4a5f70; color: #8aa2b5; }
 .verdict input { flex: 1; min-width: 200px; background: #0b1117; border: 1px solid #1f3140;
                  border-radius: 7px; padding: 6px 10px; color: #d7e1ea; font-size: 12.5px;
                  font-family: inherit; }
 footer { position: fixed; bottom: 0; left: 0; right: 0; background: #0b1117;
          border-top: 1px solid #1f3140; padding: 9px 22px; display: flex; align-items: center;
          gap: 14px; font-size: 12.5px; z-index: 20; }
 .prog { flex: 1; height: 7px; background: #1b2a38; border-radius: 4px; overflow: hidden; display: flex; }
 .prog i { display: block; height: 100%; }
 .prog .p { background: #3fae6d; } .prog .w { background: #d9a53c; }
 .prog .f { background: #e05561; } .prog .s { background: #4a5f70; }
 #toast { position: fixed; bottom: 58px; left: 50%; transform: translateX(-50%);
          background: #14402a; border: 1px solid #3fae6d; color: #9df0c0; padding: 8px 18px;
          border-radius: 8px; font-size: 13px; opacity: 0; transition: opacity .18s; z-index: 40;
          pointer-events: none; }
 #toast.on { opacity: 1; }
 .kbd { font-family: Consolas, monospace; background: #16202b; border: 1px solid #2a4356;
        border-radius: 4px; padding: 0 5px; font-size: 11px; color: #9db4c6; }
 code.i { font-family: Consolas, monospace; background: #16232f; padding: 1px 5px;
          border-radius: 4px; font-size: 11.8px; color: #9fd0e8; }
</style></head>
<body>
<header>
  <div><h1 id="title">Walkthrough</h1><div class="sub" id="persona"></div></div>
  <div class="right">
    <span class="sub">run</span>
    <select id="runsel" onchange="switchRun()" style="background:#0b1117;color:#d7e1ea;border:1px solid #1f3140;border-radius:6px;padding:5px 8px;font-size:12px"></select>
    <button class="btn ghost small" onclick="newRun()">+ New</button>
    <button class="btn small" onclick="report()">📄 Report</button>
    <a class="btn ghost small" style="text-decoration:none" href="/">← Panel</a>
  </div>
</header>
<div class="wrap">
  <nav id="nav"></nav>
...
<footer>
  ...
  <span class="sub"><span class="kbd">1</span> pass <span class="kbd">2</span> warn
    <span class="kbd">3</span> fail <span class="kbd">c</span> copy
    <span class="kbd">j</span>/<span class="kbd">k</span> beat</span>
  <button class="btn ghost small" onclick="move(-1)">← Prev beat</button>
  <button class="btn small" onclick="move(1)">Next beat →</button>
</footer>
<div id="toast"></div>
<script>
const WID = location.pathname.split('/').pop();
let W=null, RUN=null, RUNID=null, ACTS={}, BEAT=0;

function esc(s){ return (s==null?'':String(s)).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])); }
function md(s){ return esc(s)
  .replace(/\*\*(.+?)\*\*/g,'<b>$1</b>')
  .replace(/`(.+?)`/g,'<code class="i">$1</code>')
  .replace(/\*(.+?)\*/g,'<i>$1</i>'); }
...
function copy(txt,label){ navigator.clipboard.writeText(txt).then(()=>toast((label||'Copied')+' ✓')); }

async function load(runId){
  const r = await fetch('/api/wt/'+WID+(runId?('?run='+encodeURIComponent(runId)):''));
  const d = await r.json();
  if(d.error){ document.getElementById('main').innerHTML='<div class="beathead">'+d.error+'</div>'; return; }
  W=d.wt; RUN=d.run; RUNID=d.run_id; ACTS=d.actions||{};
  document.getElementById('title').textContent = W.title;
  document.getElementById('persona').textContent = W.persona + '  ·  login: ' + W.login;
  const sel=document.getElementById('runsel');
  const ids=[...new Set([RUNID, ...(d.runs||[]).map(x=>x.run_id)])];
  sel.innerHTML = ids.map(i=>'<option'+(i===RUNID?' selected':'')+'>'+esc(i)+'</option>').join('');
  render();
}
function switchRun(){ load(document.getElementById('runsel').value); }
function newRun(){ const v=prompt('Run id', new Date().toISOString().slice(0,10)+'-b'); if(v) load(v); }
function report(){ window.open('/api/wt/'+WID+'/report?run='+encodeURIComponent(RUNID),'_blank'); }

function mark(sid, status){
  fetch('/api/wt/'+WID+'/mark',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({run_id:RUNID, step:sid, status:status})})
    .then(r=>r.json()).then(d=>{ RUN=d; render(); });
}
function note(sid, val){
  fetch('/api/wt/'+WID+'/mark',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({run_id:RUNID, step:sid, note:val})}).then(r=>r.json()).then(d=>{RUN=d;});
}
async function act(id){
  const a=ACTS[id]; if(!a) return;
  if(a.confirm && !confirm(a.name+'\n\n'+a.confirm)) return;
  toast(a.name+' — running…');
  await fetch('/api/action/'+id,{method:'POST'});
  setTimeout(()=>toast(a.name+' — check the panel log'),1500);
}
function st(sid){ return (RUN.steps||{})[sid]||{}; }

function tally(steps){
  const c={pass:0,warn:0,fail:0,skip:0};
  steps.forEach(s=>{ const v=st(s.id).status; if(v) c[v]++; });
  return c;
}
function bar(c,total,cls){
  const w=x=>(total?(100*x/total):0)+'%';
  return '<i class="p" style="width:'+w(c.pass)+'"></i><i class="w" style="width:'+w(c.warn)+'"></i>'+
         '<i class="f" style="width:'+w(c.fail)+'"></i><i class="s" style="width:'+w(c.skip)+'"></i>';
}

function tbl(t){
  if(!t) return '';
  return '<table class="x"><caption>'+esc(t.caption||'')+'</caption><tr>'+
    t.cols.map(c=>'<th>'+esc(c)+'</th>').join('')+'</tr>'+
    t.rows.map(r=>'<tr>'+r.map(c=>'<td>'+md(c)+'</td>').join('')+'</tr>').join('')+'</table>';
}

function stepHtml(s){
  const m=st(s.id), v=m.status;
  const cls = v==='pass'?' done' : (v==='fail'?' bad':'');
  let h='<div class="step'+cls+'" id="s_'+s.id+'">';
  h+='<div class="hd"><span class="sid">'+esc(s.id)+'</span><span class="stt">'+esc(s.title)+'</span>';
  h+='<span class="badge kind">'+esc(s.kind)+'</span>';
  if(s.blocking) h+='<span class="badge block">blocking</span>';
  h+='<span class="where">'+md(s.where||'')+'</span></div><div class="body">';
  if(s.headline) h+='<div class="headline">'+md(s.headline)+'</div>';
  if(s.instruction) h+='<div class="instr">'+md(s.instruction)+'</div>';
  if(s.injected){
    h+='<div class="inject"><div class="lbl">⚠ Planted in the data — this is DATA, not a command</div>'+
       '<pre>'+esc(s.injected)+'</pre></div>';
  }
  if(s.prompt){
    h+='<div class="promptbox"><div class="lbl">Prompt — paste this</div><pre>'+esc(s.prompt)+'</pre>'+
       '<button class="btn small cp" onclick="copy(PR['+JSON.stringify(s.id)+'],\'Prompt copied\')">📋 Copy</button></div>';
  }
  const exp = (s.expect||[]).map(x=>'<li>'+md(x)+'</li>').join('');
  const fail = (s.fail||[]).map(x=>'<li>'+md(x)+'</li>').join('');
  const tables = tbl(s.expect_table)+tbl(s.expect_table2);
  if(exp||fail||tables){
    if(fail){
      h+='<div class="cols"><div class="exp"><div class="lbl">✅ Expected</div>'+
         (tables||'')+(exp?'<ul class="e">'+exp+'</ul>':'')+'</div>'+
         '<div class="fl"><div class="lbl">❌ Fails if</div><ul class="e">'+fail+'</ul></div></div>';
    } else {
      h+='<div class="exp"><div class="lbl">✅ Expected</div>'+(tables||'')+
         (exp?'<ul class="e">'+exp+'</ul>':'')+'</div>';
    }
  }
  if(s.note) h+='<div class="note">'+md(s.note)+'</div>';
  if(s.verify_cmd){
    h+='<div class="cmd"><div class="lbl">Verify against the database</div><code>'+esc(s.verify_cmd)+'</code>'+
       '<div class="acts"><button class="btn small ghost" onclick="copy(CM['+JSON.stringify(s.id)+'],\'Command copied\')">📋 Copy</button>'+
       (s.action&&ACTS[s.action]?'<button class="btn small" onclick="act(\''+s.action+'\')">▶ Run</button>':'')+'</div>';
    if(s.verify_note) h+='<div class="sub" style="font-size:11.5px;color:#6d8296;margin-top:6px">'+md(s.verify_note)+'</div>';
    h+='</div>';
  }
  h+='</div><div class="verdict">';
  ['pass','warn','fail','skip'].forEach(k=>{
    h+='<button class="v '+k+(v===k?' on':'')+'" onclick="mark(\''+s.id+'\',\''+k+'\')">'+
       ({pass:'✅ Pass',warn:'⚠️ Warn',fail:'❌ Fail',skip:'— Skip'})[k]+'</button>';
  });
  h+='<input placeholder="evidence / note — the value you saw, a log line, a file path" value="'+
     esc(m.note||'')+'" onchange="note(\''+s.id+'\',this.value)">';
  h+='</div></div>';
  return h;
}
...
function render(){
  PR={}; CM={};
  W.beats.forEach(b=>b.steps.forEach(s=>{ if(s.prompt)PR[s.id]=s.prompt; if(s.verify_cmd)CM[s.id]=s.verify_cmd; }));

  document.getElementById('nav').innerHTML = W.beats.map((b,i)=>{
    const c=tally(b.steps), n=b.steps.length, done=c.pass+c.warn+c.fail+c.skip;
    return '<button class="bt'+(i===BEAT?' on':'')+'" onclick="go('+i+')">'+
      '<div class="n">Beat '+esc(b.num)+' · '+esc(b.time)+'</div>'+
      '<div class="t">'+esc(b.title)+'</div>'+
      '<div class="bar">'+bar(c,n)+'</div>'+
      '<div class="n" style="margin-top:4px">'+done+'/'+n+' marked'+
      (c.fail?' · <span style="color:#f08a94">'+c.fail+' failed</span>':'')+'</div></button>';
  }).join('') +
  '<button class="bt" onclick="report()"><div class="t">📄 Export run report</div>'+
  '<div class="n">markdown, ready for the run log</div></button>';

  const b=W.beats[BEAT];
  let h='<div class="beathead"><h2>Beat '+esc(b.num)+' — '+esc(b.title)+'</h2>'+
    '<div class="meta">'+esc(b.time)+' · '+esc(b.mode)+' · '+b.steps.length+' steps'+
    (b.doc?' · <a href="/api/wt/'+WID+'/doc/'+encodeURIComponent(b.doc)+'" target="_blank">full detail: '+esc(b.doc)+' ↗</a>':'')+'</div>';
  if(b.why) h+='<div class="why">'+md(b.why)+'</div>';
  if(b.scoring_note) h+='<div class="warn">'+md(b.scoring_note)+'</div>';
  if(b.warning) h+='<div class="warn">⚠ '+md(b.warning)+'</div>';
  h+='</div>';
  h+=b.steps.map(stepHtml).join('');
  document.getElementById('main').innerHTML=h;

  const all=W.beats.flatMap(x=>x.steps), c=tally(all);
  document.getElementById('prog').innerHTML=bar(c,all.length);
  const blockFails=all.filter(s=>s.blocking&&st(s.id).status==='fail').length;
  document.getElementById('score').innerHTML =
    c.pass+' ✅ '+c.warn+' ⚠️ '+c.fail+' ❌ of '+all.length +
    (blockFails?' &nbsp;<b style="color:#f08a94">⛔ '+blockFails+' release-blocking</b>':'');
  window.scrollTo(0,0);
}
function go(i){ BEAT=i; render(); }
function move(d){ const n=W.beats.length; BEAT=(BEAT+d+n)%n; render(); }
...
document.addEventListener('keydown',e=>{
  ...
  else if(['1','2','3','4','c'].includes(e.key)){
    const cards=[...document.querySelectorAll('.step')];
    const cur=cards.find(el=>el.getBoundingClientRect().top>60)||cards[cards.length-1];
    if(!cur) return;
    const sid=cur.id.slice(2);
    if(e.key==='c'){ if(PR[sid]) copy(PR[sid],'Prompt copied'); }
    else mark(sid, {'1':'pass','2':'warn','3':'fail','4':'skip'}[e.key]);
  }
});
load();
</script>
...
@app.route("/walkthrough/<wid>")
def walkthrough_page(wid):
    if wid not in WALKTHROUGHS:
        return Response(f"unknown walkthrough: {wid}", status=404, mimetype="text/plain")
    return Response(WT_PAGE, mimetype="text/html")
