# Platform Regression Report — 20260903_183249 (local dev)

- Build: `907df6d` | Base: `http://localhost:5001` | Baseline: `results_20260903_183117.json`

## Verdict: **CLEAN** — 51 PASS / 51 SKIP / 4 XFAIL
...
| Agents | agent_crud | ✅ PASS | id=1019, listed=True, deleted=True |
...
| Knowledge/Docs | knowledge_ingest_delete | ✅ PASS | agent=1021, ingest=success, chars=2450, type=vendor_guide, deleted=True |
...
| Connections | conn_create_and_list | ✅ PASS | id=577, server=10.0.0.6, db=ERPDB, user=ai_user |
...
| Data Explorer | de_pin_dashboard | ✅ PASS | 4 pins via 4 controls -> panel opened + toast each time; refresh re-ran 2 SQL tiles; unsaved-changes guard asked on '+' and on sidebar switch (keep/discard both honoured), silent when clean; saved 'REGP-pin-20260903_183249' -> rel |
...
| Workflow engine | wf14:human_approval_approve | ✅ PASS | status=completed; decided=True (req=21F8A48C-4762-47DA-82AA-850A69DE1A1D, http=200); post-approval file=True |
...
| Workflow engine | wf14:file_transfer_sftp_upload | ⏭ SKIP | env missing: ['sftp'] — SFTP test rig not running (Start_SFTP_Server.bat) |
