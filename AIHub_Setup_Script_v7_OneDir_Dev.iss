...
OutputBaseFilename=AIHub.Setup.v2.1
Compression=lzma
...
; The loose _build_config is the TRIMMED (LLM-keys-only) copy: browser-use only needs the
; Azure/OpenAI keys, so the DB password / comm-service / Flask secret are NOT written to a
; readable .py on the client disk. The FULL _build_config is still baked inside every frozen
; exe (frozen services import their copy before {app} joins sys.path, so this loose trimmed
; file only resolves for the source-run browser-use service). Generated by
; scripts/generate_build_config.py; DestName renames it to the module name Python imports.
...
begin
  ...
  begin
    ...
    begin
      // Returns the LAST assignment of Key, not the first. python-dotenv lets a
      // later line win over an earlier one, so the last line is what the running
      // platform actually resolves — reading the first one made this function
      // disagree with the app. That mismatch broke ForceEnvKeyValue's
      // idempotency: on a .env holding an early THE_AGENT_ENABLED=false plus the
      // line a previous upgrade appended, this returned 'false', so EVERY
      // upgrade appended another overriding block (v5 and earlier accumulated
      // one duplicate pair per run). Do not re-add the Break.
...
begin
  ...
  begin
    ...
    begin
      ...
      // --- CC_ROUTE_MEMORY (v1.8.2) ---
      // Route memory (learned query->agent shortcuts) ships DISABLED: its
      // success-judged learning can memorize a confidently-wrong route from
      // a single plausible-looking answer. The code default is true, so the
      // key must exist as false in every install's .env.
      if not EnsureEnvKeyExists(EnvConfigFile, 'CC_ROUTE_MEMORY', 'false') then
      begin
        MsgBox('Warning: Failed to write CC_ROUTE_MEMORY to .env.' + #13#10 +
               'You may need to add it manually',
               mbError, MB_OK);
      end;

      // --- CC_INSPECT_ENVIRONMENT (v1.8.2) ---
      // Kill switch for the CC read-only environment-inspection path
      // (schema/agent/secret-name/MCP questions answered by CC's own tools).
      // Mirrors the code default (true); seeded so operators can find and
      // flip it without a deploy.
      if not EnsureEnvKeyExists(EnvConfigFile, 'CC_INSPECT_ENVIRONMENT', 'true') then
      begin
        MsgBox('Warning: Failed to write CC_INSPECT_ENVIRONMENT to .env.' + #13#10 +
               'You may need to add it manually',
               mbError, MB_OK);
      end;

      // --- CC_PICKER_CONNECTIONS (v1.8.2) ---
      // Kill switch for agent-picker connection linkage ("queries connection:
      // X" in routing prompts). Mirrors the code default (true).
      if not EnsureEnvKeyExists(EnvConfigFile, 'CC_PICKER_CONNECTIONS', 'true') then
      begin
      ...
      // nav entry and no routing. Force BOTH (an appended line overrides).
      //
      // v7 CHANGED THE BLAST RADIUS of these two flags. Through v6 they were
      // safe on their own because the nav entry, /the-agent and the '/' redirect
      // were all still gated to Developers/Admins by AGENT_ALLOW_ALL_USERS=false.
      // v7 forces that key to TRUE (below), so THE_AGENT_ENABLED +
      // THE_AGENT_MODE now reach EVERY signed-in user, and '/' lands regular
      // users on The Agent rather than the classic dashboard. Setting
      // AGENT_ALLOW_ALL_USERS back to false restores the Developer+ preview
      ...
      // --- AGENT_ALLOW_ALL_USERS (v7: all-users rollout) ---
      // v5/v6 seeded 'false' with EnsureEnvKeyExists, keeping The Agent in
      // preview for Developers/Admins (role>=2). v7 opens it to every signed-in
      // user, so FORCE 'true': an upgrade keeps the client's own .env, and
      // Ensure would have preserved a pre-existing 'false' forever.
      //
      // This flag is read on BOTH sides of the hop and one .env feeds both —
      // agent_service/agent_config.py loads {app}\.env explicitly:
      //   main app  app.py    '/' redirect, /the-agent route, nav entry
      //   service   main.py   'if role < 2 and not AGENT_ALLOW_ALL_USERS'
      // so this single key is what actually opens the door; the route gate
      // alone opens nothing while the service still refuses role-1 users.
      //
      // NOTE: this also changes where regular users LAND. With THE_AGENT_MODE
      // true, '/' sends any signed-in user to The Agent instead of the classic
      // dashboard. ?classic=1 remains the sticky escape hatch, and setting this
      // key back to false restores Developer+ only.
      if not ForceEnvKeyValue(EnvConfigFile, 'AGENT_ALLOW_ALL_USERS', 'true') then
      begin
        MsgBox('Warning: Failed to write AGENT_ALLOW_ALL_USERS to .env.' + #13#10 +
               'You may need to set it manually: AGENT_ALLOW_ALL_USERS=true',
               mbError, MB_OK);
      end;
      ...
      // v6: FORCE, not Ensure. EnsureEnvKeyExists preserved any existing value,
      // so an install already carrying NLQ_ENGINE_DEFAULT=legacy stayed on the
      // old engine through every upgrade, silently. Same reasoning as
      // THE_AGENT_ENABLED above. NOTE: this overrides a deliberate opt-out too —
      // a client who must stay on legacy should be pinned per-agent with
      // NLQ_LEGACY_AGENT_IDS, or have this line edited after setup.
      if not ForceEnvKeyValue(EnvConfigFile, 'NLQ_ENGINE_DEFAULT', 'agentic') then
      begin
        MsgBox('Warning: Failed to write NLQ_ENGINE_DEFAULT to .env.' + #13#10 +
               'You may need to set it manually: NLQ_ENGINE_DEFAULT=agentic',
               mbError, MB_OK);
      end;
      ...
      // v6: FORCE, for the same reason as NLQ_ENGINE_DEFAULT directly above —
      // these two are a matched pair and fixing only one leaves the same silent
      // downgrade on the document-search side.
      if not ForceEnvKeyValue(EnvConfigFile, 'DOC_SEARCH_ENGINE_DEFAULT', 'v2') then
      begin
        MsgBox('Warning: Failed to write DOC_SEARCH_ENGINE_DEFAULT to .env.' + #13#10 +
               'You may need to set it manually: DOC_SEARCH_ENGINE_DEFAULT=v2',
               mbError, MB_OK);
      end;
    ...
    begin
      ...
      try
        ...
        ConfigText := #13#10 +
          ...
          // v1.8.2: same defaults the upgrade path seeds via
          ...
          // v7: NOT a redundant duplicate — a deliberate OVERRIDE. A fresh
          // install receives dist\.env verbatim (see [Files], Flags:
          // onlyifdoesntexist) and that file still ships
          // AGENT_ALLOW_ALL_USERS=false. This ConfigText is APPENDED to it and
          // python-dotenv takes the LAST assignment, so this line is what makes
          // a FRESH install match the upgrade path's ForceEnvKeyValue above.
          // Without it, upgraded clients would open The Agent to everyone while
          // brand-new ones silently stayed Developer/Admin-only.
          'AGENT_ALLOW_ALL_USERS=true' + #13#10;
          // Rule for everything else: do NOT add the remaining upgrade-path keys
          // here. dist\.env already carries THE_AGENT_ENABLED, THE_AGENT_MODE,
          // BROWSER_USE_* and WORKFLOW_TRAINING_CAPTURE_PATH at the values we
          // want, so repeating them would add duplicate lines for no effect.
          // Add a key here only when dist\.env's value must be overridden, as
          // above. The upgrade path needs its own Ensure/Force calls because an
          // upgrade keeps the client's OWN .env and never receives dist\.env.
