...
| **PB — sizing the brute-force gate** | Make the threshold a **character/token budget, not a page count**, and confirm which model the knowledge path resolves to. Today an agent can send 326K tokens uncapped below the line while the code's own "safe" cap is 100K | No agent silently exceeds the resolved model's context; per-question token cost is visible |
| **PC — dev/prod parity** | Dev `.env` is pinned to 5 while production runs 999, so dev and production exercise different paths. Decide deliberately what dev should test | Dev can run either path on purpose, not by accident |
| **P1** | Labeled fixture corpus + battery + both-engines runner; score **legacy** to establish the baseline | Baseline recall/precision published; the traps in §7.3 all have known answers |
| **P2** | `doc_search_factory` + config keys + call-site swaps + breaker, legacy-only behavior | Legacy behavior provably unchanged; unit tests green |
| **P3** | **Strategy B (SWEEP)** + coverage ledger + cost confirmation | Recall ≥ 0.95, coverage honesty 100% on the 40-doc corpus; then re-run at 500 docs |
