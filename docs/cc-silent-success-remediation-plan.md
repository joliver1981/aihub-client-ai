...
- **W3 — Rewire stale build consumers (#15, #14) · medium · low.** #15: converse tool `delegate_to_builder_agent` (`command_center_service/graph/nodes.py:1683`) returns only `result["text"]`, discarding the honest status/plan; consumer `:3742` hardcodes `in_progress`; a *third* consumer (gather_data `:4426-4436`) is also unrewired. *Fix:* factor the build node's derivation (`:6169-6204`) into a shared helper used by all three. #14: the classifier branch (`builder_service/graph/nodes.py:4189`) closes a failed compile as `completed` on the LLM's word (its `planning`-only guard never matches a post-compile error). *Fix:* broaden the guard to keep the conversation active when `compile_result.status not in (success, draft)`.
- **W4 — Intent-aware verification (#18) · small–med · low.** `create_mcp_server` (`app.py:16575`) defaults `server_type='local'` when omitted (URL server written as a broken local row; url dropped); `transport_type` never read; `_check_mcp_create` matches name/url only. *Fix:* default `server_type="remote"` in the schema (mirrors the Phase-1 `mcp.test_server` fix); extend `_check_mcp_create` to assert the created row's `server_type` matches; optionally persist `transport_type`.
- **W5 — Fail-closed the success direction (#17) · small · low.** On a distiller LLM exception, `command_center_service/graph/nodes.py:6150` shows the generic "couldn't format it safely" apology even on a fully-verified success (the Phase-4 footer only emits for failed/unverified). *Fix:* in the fallback, synthesize a deterministic success message from `verification_summary` + `latest_plan` (`_extract_created_resources`) before resorting to the apology.
