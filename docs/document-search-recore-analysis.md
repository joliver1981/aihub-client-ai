...
| **P1** ✅ **COMPLETE 2026-07-26** | **Pack 13** (`test_human/13_Document_Competency/`) = labeled battery + live runner + **scale harness**: `gen_scale_corpus.py` (120 synthetic leases, exact ground truth, synonym/split/silent traps), `load_scale_corpus.py` (live SQL + vector shapes, real `index_knowledge_document`), `run_scale.py` (store-ID recall grading) | Small-scale legacy baseline 86%/83% (C2); **scale head-to-head: legacy 97.5% recall w/ an invisible silent miss vs v2 SWEEP 100%/100% with ledger** — see `SCALE_RESULTS.md` |
...
| **P3** ✅ **BUILT + SCALE-VALIDATED (1105b64 + c2199b6 + map-prompt fix 2026-07-26)** | SWEEP: whole-document map (proxy Haiku, strict-JSON per doc, parse-fallback never dropped) → deterministic reduce → **coverage ledger**; cost estimate + confirm-above-threshold; NEEDLE defers to legacy via mini-LLM router. **Doctrines:** (1) agents decompose portfolio questions into per-doc NEEDLE-shaped tool calls — route AND extract on `latest_user_input`, cache one sweep per (agent, user, question, **active-doc-set fingerprint**); (2) filtered-list questions degenerate per-document maps to bare identifiers — the map prompt mandates **'YES —'/'NO —' condition semantics** with substance per document | ✅ Small scale: C2 100%/100% vs legacy 86%/83%. ✅ **120-doc scale: v2 100% recall / 100% precision, all synonym-only found, ledger relayed, 47s/$0.24 sweep — vs legacy 97.5% recall with an INVISIBLE silent miss and no ledger** (`SCALE_RESULTS.md`) |
| **P4** ✅ **BUILT — knowledge path 2026-07-26** | `doc_search_v2/needle.py`: hybrid always-both — page-level **pure-Python BM25** (deviation from the planned DuckDB FTS: the extension downloads on `INSTALL fts` — a client-box risk — and agent corpora ≤~1K pages don't need it) + the existing gated dense channel, fused with RRF (k=60, chunk hits vote for their page), Haiku rerank failing OPEN to RRF order, citation-ready `SOURCE [filename p.N]` evidence blocks + retrieval ledger; per-agent index cached by active-doc-set fingerprint; `DOC_NEEDLE_V2_ENABLED` kill switch (needles defer to legacy when off). Native `search_result`/citations blocks deferred to the relay work | ✅ 29/29 unit tests; **live: 12/12 per-store fact checks on the 120-doc corpus (exact ground-truth values, source cited in every answer)** + pack-13 C1 PASS via `PATH: DOC_SEARCH_V2 (needle)`; engine-labeled tracing |
| **P5** ✖ **DROPPED (james, 2026-07-26)** | FACTS (extract-once fact store) rejected as introducing too much complexity (schema authoring/versioning, staleness policy, second query surface). SWEEP covers the exhaustive class; its (agent, question, doc-set) cache already serves repeat questions within TTL | Not planned. Revisit only if a client's repeated-portfolio-question volume makes per-question sweep cost material |
