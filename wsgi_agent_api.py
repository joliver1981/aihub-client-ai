...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.getenv('AGENT_API_LOG', _default_log), encoding='utf-8'),
...
port = int(os.getenv('HOST_PORT', 5001)) + 40
...
def start_server():
    ...
    logging.info(f"Starting production Agent API server on {host}:{port} with {threads} threads (channel_timeout={channel_timeout}s)")
    serve(
        app,
        host=host,
        port=port,
