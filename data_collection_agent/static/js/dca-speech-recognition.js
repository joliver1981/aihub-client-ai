 ...
 *
 * NOTE: This file is a copy of /static/js/speech_recognition.js shipped under
 * the data_collection_agent blueprint. The platform's original CommonJS export
 * doesn't attach to `window`, which breaks under SES / MetaMask hardened JS
 * (top-level `class X` declarations don't leak to globalThis in those
 * environments). The only difference vs. the platform file is the explicit
 * `window.SpeechRecognitionManager = SpeechRecognitionManager;` at the bottom.
 *
 * If the platform file gets enhancements you want here, copy it again and
 * re-append the window-attachment line.
 */
...
// Explicit window attachment (DCA addition).
// Top-level `class X` declarations don't auto-attach to globalThis in
// SES/MetaMask hardened-JS environments — without this line, separate
// scripts (like voice-mode-streaming.js) can't reference the class.
