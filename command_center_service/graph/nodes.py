...
async def converse(state: CommandCenterState) -> dict:
    ...
    if _AUTOMATIONS_TOOLS_ENABLED and _automations_allowed(state):
        _automations_prompt = (
            ...
            "contain credential literals are rejected. For SQL, prefer "
            "aihub.query('CONN_NAME', sql, params) — it returns rows as dicts and handles the "
            "connection for you.\n"
            "- PLATFORM AI (non-negotiable): when generated code needs an LLM, call "
            "aihub.llm(prompt, system=None, images=[...], model=None) for plain text or "
            "aihub.ai_extract(prompt, images=[...], schema={...}) for parsed-JSON extraction "
            "(vision: pass workdir-relative image file paths, e.g. a PDF page rendered to PNG). "
            "The PLATFORM supplies the API key and resolves the model centrally — scripts must "
            "contain NO API key and NO hard-coded model id; leave model unset (or expose an "
            "optional 'model' input defaulting to '' = platform default).\n"
            "- APPROVAL ROUTING & EVIDENCE: aihub.checkpoint(message, files=[...], assignee=None, "
            "assignee_group=None) PAUSES the run and lands in My Approvals AND Mission Control — "
            "attach the evidence the approver needs (the CSV, the report) via files (workdir-"
            "relative). Route with assignee_group='Group Name' (any member decides) or assignee="
            "user id/username; default is the run's requester. Expose routing as inputs (e.g. "
            "approvals_group) rather than constants.\n"
            "- PER-ITEM EXCEPTIONS DON'T PAUSE: in a batch, kick each exception document/row to "
            "the queue with aihub.review_item(message, title=..., files=[the problem file], "
            "assignee/assignee_group) and KEEP GOING — one blocking checkpoint per BATCH at most. "
            "NEVER checkpoint per document. Read those verdicts back with "
            "aihub.review_decisions(request_ids) -> {request_id: 'pending'|'approved'|'rejected'|"
            "'cancelled'|'missing'}; it returns None when the queue is unreachable, which you must "
            "treat as still-pending and fail SAFE (never send/act on an undecided item). "
            "AFTER the batch has applied the decisions, write back what each decision DID with "
            "aihub.review_outcome(request_id, code, label=..., note=...) — 'included', "
            "'acknowledged', 'rejected', 'correction_refused' (say WHY and what to change), "
            "'undecided' — so the reviewer's history never shows a bare 'Approved' for a "
            "document that was not actually processed.\n"
            "- SENDING EMAIL: aihub.send_email(to, subject, body='', html_body=None, files=[...]) "
            "sends THROUGH the platform and continues — the platform holds the mail credentials, "
            "so the script carries none (same seam as aihub.llm for model keys). `to` is an "
            "address, a list, or a ';'/','-separated string; `files` are workdir-relative "
            "attachments. This is the ONLY way to send mail from an automation — there is no "
            "aihub.email() and no smtplib.\n"
            "- ALSO AVAILABLE: aihub.log(message) for a progress line in the run log, and "
            "aihub.inputs() for this run's resolved inputs as a dict (manifest defaults applied) "
            "when you want them all rather than aihub.input('name') one at a time.\n"
            "- INPUT DESCRIPTIONS: give every manifest input a 'description' — it renders as help "
            "text in the Settings panel and the workflow designer. Make tunables (thresholds, "
            "folders, namespaces, routing) INPUTS with sensible defaults, not constants in code.\n"
            "- EMPTY RESULTS AREN'T FAILURES: for 'flag/report whatever matches' outputs, an "
            "EMPTY file is a legitimate outcome — declare min_rows only when emptiness is "
            "genuinely abnormal for the process. When a batch finds nothing, say so plainly and "
            "prefer skipping the external upload/send over shipping an empty file; never let a "
            "legitimately-empty report mark the run failed.\n"
            "- SECRET RETURN SHAPE: aihub.secret('NAME') returns the secret VALUE as a plain "
            "STRING — never call .get() on it. Transfer secrets hold either a URL "
            "('sftp://user:pass@host:2222') or a JSON string; PARSE accordingly: try "
            "json.loads(value) first, else urllib.parse.urlparse(value) for "
            "hostname/port/username/password (the runner's own verifier uses exactly these two "
            "formats).\n"
            "- FORMATTING: for build summaries use markdown headers per section and a TABLE for "
            "column lists (name | purpose); put values like paths and defaults in inline code. "
            "Avoid long flat bullet runs.\n"
            ...
            "trustworthy. Timestamped filenames may be declared as a glob ('report_*.csv' — the "
            "newest match is verified). A run can be success / failed / unverified / skipped — "
            "report the outcome verbatim, never soften a failed or unverified run into success.\n"
...
try:
    try:
        ...
        for _po_i in range(6):
            ...
            _po_dir = _po_parent
    if _po_apply:
        _po_apply(globals(), 'command_center_service/graph/nodes.py')
except Exception:
    pass
