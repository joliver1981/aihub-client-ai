...
**Status:** IN PROGRESS (2026-07-14). Decisions LOCKED by James. **v0 atom BUILT** (commit 8315b62): the inline **Code Step** node runs LLM-authored Python through the shared runner executor (`_execute_code` / `run_code_step`) with env + SDK + verification + live events + honest tri-state, and the engine dispatches it with pass/fail edges. The Automations asset path is byte-for-byte unchanged (thin wrapper over the same core; 90 automation tests still green).

**AUTHORING LAYER BUILT** (2026-07-14, next commit after 22a9c79): the agent can now author a Code Flow end to end.
- `codeflows/compiler.py` — `compile_to_workflow(defn)` turns a definition (`{name, steps[], edges[], start?}`) into the exact engine `workflow_data` (Code Step nodes, flat `source/target/type` edges, `isStart`, `kind='code_flow'`); `dry_run_walk(defn, runner, workdir)` executes the graph **in-process** via `runner.run_code_step`, honoring pass/fail routing and passing files between steps via `${<step_id>_files[0]}` substitution. Verified parity with the engine: `_execute_code_step_node` calls the same `run_code_step`, and `_replace_variable_references` supports `${var[0]}`, so the interactive walk mirrors the scheduled path.
- `codeflows/manager.py` — definition CRUD; a flow is PERSISTED as a workflow (Workflows table, `kind='code_flow'`, MERGE by name) with the authored definition **embedded in the same blob** (one source of truth for edit + run). Compile-on-save, save-time credential scan (reuses `scan_for_secrets`), stubbable `_db_*` seams.
- `codeflows/api.py` — Developer-gated REST + `internal_manage` dispatch (X-API-Key + authoritative role≥2), blueprint wired into `app.py`. **Scheduling reuses the EXISTING `workflow` scheduler job type** (`TargetId` = the flow's workflow id) — no new job type, no scheduler restart.
- CC tools (`nodes.py` + `codeflow_tools.py`): `create_code_flow`, `add_code_step`, `wire_steps`, `update_step_code` (editable), `get_code_flow`, `dry_run_code_flow`, `run_code_flow`, `schedule_code_flow` — dual-registered (tools.append + tool_map; the AIHUB-0028 trap) and guarded by a static invariant test. Build-shape classifier + system prompt teach single-script Automation vs multi-step Code Flow within the family.
- Tests: 53 green (manager 19, API dispatch+gates 13, compiler+walk 10, atom 9, invariant 2). **Needs main-app + CC restart to go live; not yet e2e'd against the real DB/engine.**

**ADVERSARIAL REVIEW + FIXES** (2026-07-15, commit dc347ba): a 5-dimension adversarial review (25 confirmed findings) surfaced one real ship-blocker and a cluster of walk↔engine parity gaps — all fixed:
- **[HIGH] auth bypass** — a code flow is an ordinary `Workflows` row, so `/api/workflow/run` + `/delete/workflow` let any role-1 session user trigger/delete it. Now gated (`_deny_code_flow_for_non_developer` in app.py) for sub-Developer SESSION callers; API-key/scheduler stays trusted.
- **honesty** — `dry_run_code_flow`/`run_code_flow` now say plainly they EXECUTE for real with live credentials (no sandbox); `summarize_walk` annotates a fail-edge-handled "success" so it can't read as clean.
- **parity** — scheduled `variables` now forwarded through `workflow_client.start_workflow`; the walk's `${var}` whole-object substitution stringifies dict/list to match the engine; `allowUnverified` added to the code-step node + walk (mirrors the Automation node — fixes the SFTP-upload-always-fails case); engine reads the human label from the node top level.
- **correctness** — max_steps truncation → 'failed' (not 'success'); per-visit dry-run workdir; parse-independent `_db_exists` dup-guard + kind guards on load/get; in-process edit lock; client timeout sized to declared per-step timeouts.
- **tests** — +27 (80 code-flow tests green), incl. the first BEHAVIORAL tests of the engine's `_execute_code_step_node` (the scheduled path), schedule SQL-writer, and summarize_walk honesty.

Still to build: expanded read-only SDK (`query_data`/`ask_agent`/`notify`), canvas code-edit view (frontend), inter-step shared workdir (v0 passes files by absolute path via variables). Known follow-ups from the review (deferred, lower value): a genuine no-egress sandbox dry mode; a run-handle+poll API so a long real run is never misreported; cross-process optimistic-version guard on the MERGE.
