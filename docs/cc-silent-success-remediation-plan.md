...
- **W2 — Node-type integrity (#6/#10) · medium · medium — STAGED (revised plan, 2026-07-08).** Runtime `workflow_execution.py:572-578` returns `{'success':True}` for unknown/`'Server'` nodes; the production validator had no node-type-membership check; the canonical list `system_prompts.py:2775` listed `'Server'` (no runtime handler) and omitted `'Portal'` (implemented at `workflow_execution.py:566`).
  - **Step 1 DONE** — fixed `system_prompts.VALID_WORKFLOW_NODE_TYPES` to match the runtime (swap `'Server'`→`'Portal'`; `'Human Approval'` verified real via the pause path at `workflow_execution.py:509`) + accurate source-of-truth comment. Minimal, single-line list change.
  - **Step 2 DONE** — added `detect_unknown_node_type` to `workflow_deterministic_validator.py` (sources the corrected canonical set lazily/defensively; ERROR, **no fixer** → `unfixable_errors` → `is_valid=False` → saved as DRAFT with errors surfaced, never "ready to use"). **Validator-only — no execution change.** +8 harness tests.
  - **Step 3 (audit existing workflows) — SKIPPED** per owner decision.
  - **Step 4 (runtime hard-fail) — DONE (2026-07-08, owner-approved after reviewing the exact change).** `workflow_execution.py:572-578` else-branch now returns `{'success': False, 'error': "...is not implemented..."}` instead of `{'success': True}`, so an unimplemented/unknown node is marked FAILED (the existing `if not result.get('success'): raise` at ~584 routes it through the same fail-path as any other node failure) instead of a silent passing no-op. This is the execution-layer backstop for existing/hand-edited workflows the compile-time validator (step 2) never re-checks. +1 source-contract guard. **Changes the live execution engine + existing workflows** (a workflow with an unknown/`'Server'` node now fails where it used to falsely "complete").
- **W3 — Rewire stale build consumers (#15, #14) · medium · low.** #15 **(W3a DONE 2026-07-08):** converse tool `delegate_to_builder_agent` returned only `result["text"]`, discarding the honest status/plan; consumer `:3742` hardcoded `in_progress`; a *third* consumer (gather_data `:4426-4436`) was also unrewired — so chat-path builds never marked complete (the session stayed pinned to the builder, created resources never surfaced). *Fixed:* added a shared `_derive_build_state(result)` helper (build_status/created_resources/completed_at/builder_session_id from the plan); the converse tool now stashes the full result in a turn-scoped `_builder_capture` and its handler + the gather_data continuation both derive from it. Build node left as-is (its `latest_plan` accounts for the auto-confirm `result2` — refactoring it would risk the live-verified path). +8 harness tests. #14 **(W3b DONE 2026-07-08):** the classifier branch closed a failed compile as `completed` on the LLM's word (its `planning`-only guard never matched a post-compile error), dropping the thread. *Fixed:* extracted `_workflow_turn_should_stay_active(agent_id, phase, has_definitive_result, compile_status)` — keeps the conversation active when it's plan-only OR the compile status is present and not in (success, draft); +9 harness tests. **W3b-2 closer (2026-07-08, retest #14):** the retest found a *second* completion path — `manager.py:250-268` `_analyze_agent_response` marks the conversation `completed` on the agent's **prose** ("i've created…") regardless of the real compile outcome, feeding the earlier `status=='completed'` branch my first guard didn't cover. Fixed by computing `_keep_active` once and checking it **before** every completion trigger (manager-status, has_definitive_result, and the classifier), so a workflow_agent whose compile didn't succeed is never closed. +1 wiring guard. (#15 still pending.)
...
#### Phase 6 — Round 2: adversarial re-verification + residual closes (2026-07-09)
Second post-restart retest: **4 of the 6 Bucket-A findings fully closed** end-to-end (#6/#10 W2, #15 W3a, #14 W3b-2); **3 landed with a narrow residual** (#8, #17, #18). A 4-agent adversarial pass re-read each residual against current code and **CONFIRMED all three** as genuine same-class instances (minor line-number corrections only), and scoped the 6 "Bucket B" items. All four closes below are regression-locked (+12 harness tests; suite now 122 passing).

- **#8 (medium) — timeout-clobber on the auto-reply *retry* sub-path.** W1 fixed the primary first-send timeout, but a deeper path remained: a timed-out auto-reply retry send hits the no-metadata else, which called `mark_waiting_for_user`, and that **unconditionally overwrote `TIMEOUT`→`WAITING_FOR_USER`** (`builder_service/agent_communication/models.py:99`) *before* the `_conversation_failure_result` guard read the status (`builder_service/graph/nodes.py:3635`) — guard missed it → `success=True`, build never done, no error. *Fix (root-cause):* `mark_waiting_for_user` now refuses to downgrade a terminal `TIMEOUT`/`FAILED` (a terminal failure is sticky), protecting the guard and every other caller. +2 tests (behavioral + source-guard).
- **#17 (low) — plan-only compile path + distiller crash still apologized.** W5b covered the top-level `saved_workflow_id`, but the *plan-only* compile path stores the id only at `result["compile_result"]["workflow_id"]`, which `_extract_created_resources` never read — so a clean plan-only build + a distiller crash showed the generic apology. *Fix:* `_extract_created_resources` now also reads `compile_result.workflow_id`/`workflow_name`, gated on a clean (non-draft/error) compile so drafts stay suppressed (`_plan_has_unready_artifact` still independently gates the announcement). +3 tests.
- **#18 (low, honesty) — `transport_type` silently dropped.** W4 closed the marquee harm (broken local row); the residual: `mcp.create_server`/`mcp.test_server` collected a **required** `transport_type` (sse/streamable_http) that the `app.py` handlers never read/store/verify (grep: zero `transport` in `app.py`) — a dropped selection reported as success (masked in practice by gateway auto-negotiation, StreamableHTTP-first/SSE-fallback). *Fix (honest, owner-chosen):* removed the `transport_type` field from both action schemas — don't offer a knob we can't honor. Re-add only alongside real persistence + verification (pattern exists at `builder_mcp/routes/mcp_routes.py:171`). +2 tests. (Supersedes the W4 "deferred `transport_type`" note above.)
- **#11 (Bucket B, medium, honesty) — email auto-reply approval stub.** The one Bucket-B item that is genuinely silent-success — but on the **inbound-email** subsystem, not the CC→Builder path: `_queue_for_approval` (`email_agent_dispatcher.py:620`) returned `{success:True, pending_approval:True}` while drafting/persisting/queuing **nothing**, and the dispatcher then told the user "an auto-reply was drafted … and is pending approval" (false). *Fix (honest, defer the feature):* `_queue_for_approval` now returns an honest not-implemented failure, and the auto-reply notification distinguishes sent / could-NOT-send / pending (the old `else` claimed "was sent" even when `status=='failed'`). A real draft+approval workflow (drafts table + retrieve/approve/send endpoints + UI) is the deferred feature. +2 tests.

**#11 superseded (2026-07-09) — Agent Email Approvals feature REBUILT (not just honesty-stubbed).** The deferred feature behind #11 was rebuilt after discovery found the backend data+send layer had already survived (`agent_email_send.py`, recovered in `60c3fb6`, with passing test oracles). What was lost — and is now rebuilt: (a) `DataUtils.accessible_agent_ids(user_id, role)` (a co-lost group-based authz helper that also silently broke `/api/agents/summary`'s visibility filter; None=admin, list=scoped, []=deny-all fail-closed); (b) the `/api/agent-email/approvals` routes (list/get/act) in the `agent_email_bp` blueprint, thin wrappers over the surviving send layer with real `@api_key_or_session_required(min_role=2)` auth + per-agent scoping (approve ACTIVELY sends via `send_approved_email` — no poller); (c) the "Agent Email Approvals" section + Workflow⇄Email toggle in `templates/approvals.html` (editable `final_body` modal); (d) the dispatcher wiring — `_trigger_auto_response` now generates the draft then routes through `agent_email_send.send_agent_email` (queue-vs-send gate), and the `_queue_for_approval` honesty-stub was DELETED (this is the "undo #11"). Also un-hid/un-neutered the config-page `require_approval` switch and fixed the notification silent-no-op (require a recipient in the UI). The dispatcher rewire is pinned by the surviving `tests/unit/test_email_agent_dispatcher.py::TestAutoResponseGate`; `accessible_agent_ids` + the chokepoint wiring got new regression tests. So `require_approval=True` now produces a real pending `AgentEmailApprovals` draft a human reviews/edits/sends from My Approvals.

**Bucket B disposition (5 of 6 stay separate reliability work, not silent-success):** #5 is subsumed by the already-fixed #15 (session-pinning) and has a working REROUTE escape hatch; **#9 does not reproduce** (no `.created_resources` attribute access exists — the dict is type-safe + try/except-guarded); #12 is a context-quality nit (goal used only for auto-reply enrichment, never gates success); #13 is a read-only query-accuracy bug (substring ID match fetches the wrong entity, no mutation); #19 (StreamConsumed in the `text_chat` adapter) is an error-**masking** bug (the real agent error is still surfaced as FAILED, just replaced with a confusing message). Recommendation: batch #19+#13 into a general reliability sweep; verify-and-close #5+#9; #12 is low. The silent-success scope boundary held under adversarial reading.
