# Platform Regression Report — 20260905_100732 (local dev)

- Build: `c4c5dc6` | Base: `http://localhost:5001` | Baseline: `results_20260903_194844.json`

## Verdict: **CLEAN** — 52 PASS / 50 SKIP / 4 XFAIL
...
| Agents | agent_crud | ✅ PASS | id=1029, listed=True, deleted=True |
...
| Knowledge/Docs | knowledge_ingest_delete | ✅ PASS | agent=1031, ingest=success, chars=2450, type=customer_vendor_requirements, deleted=True |
...
| Secrets | secrets_list | ✅ PASS | secrets=83, has-test-secret=True |
...
| Connections | conn_create_and_list | ✅ PASS | id=588, server=10.0.0.6, db=ERPDB, user=ai_user |
...
| Data Explorer | de_pin_dashboard | ✅ PASS | 4 pins via 4 controls -> panel opened + toast each time; refresh re-ran 2 SQL tiles; unsaved-changes guard asked on '+' and on sidebar switch (keep/discard both honoured), silent when clean; saved 'REGP-pin-20260905_100732' -> rel |
...
| Workflow engine | wf14:human_approval_approve | ✅ PASS | status=completed; decided=True (req=535F6A5C-C126-43E4-83F8-AF2B196C11F7, http=200); post-approval file=True |
...
| Workflow engine | wf14:file_transfer_sftp_upload | ✅ PASS | status=completed; remote-file=True (xfer_20260905_101251.txt); content-ok=True |
