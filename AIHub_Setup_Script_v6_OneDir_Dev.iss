    ...
    begin
      ...
      try
        ...
        ConfigText := #13#10 +
          ...
          // NOTE (v6): do NOT add the remaining upgrade-path keys here.
          // A fresh install already receives dist\.env verbatim (see [Files],
          // Flags: onlyifdoesntexist), and that file carries THE_AGENT_ENABLED,
          // THE_AGENT_MODE, AGENT_ALLOW_ALL_USERS, BROWSER_USE_*, and
          // WORKFLOW_TRAINING_CAPTURE_PATH already. This ConfigText is APPENDED
          // to that file, so anything repeated here becomes a duplicate line.
          // The keys above are themselves already duplicates of dist\.env —
          // harmless (python-dotenv takes the last assignment, and this block
          // lands last) but not worth extending. The upgrade path needs its
          // Ensure/Force calls because an upgrade keeps the client's OWN .env
          // and never receives dist\.env.
