...
| 1 | **Coverage as data, in front of the model** (no inspection of the reply's wording): the data tools keep a per-turn ledger of which connections exist and which ones a row-level query touched; every probe result ends with one names-only line (*"Queried this turn: …. Not queried: …."*, no repeated doctrine), so the boundary of the search is stated at the moment the model is about to conclude. New **`search_tables`** tool: one call lists candidate tables across every connection and reports matches, no-match connections, and connections it could not list (unchecked, not empty) — the capability that was missing when the model walked 2 of 6 connections by hand. | `platform_tools.py` (`coverage_footer`, `search_tables`) |
