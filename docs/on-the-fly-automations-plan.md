...
**Status:** ALL PHASES (P0–P5) BUILT 2026-07-13. Migration 014 APPLIED to the platform DB by James; full lifecycle live-verified against the real platform DB. Tester (openclaw) approved AIHUB-0027 (16/17; findings F1–F3 fixed: remote_path alias, version in run responses, ASYNC environment provisioning).

**AUTOMATION STUDIO (experience layer, Phases A–C) BUILT 2026-07-13** per James's decisions (docked panel in CC; CC always proposes checkpoints behind `CC_AUTOMATIONS_PROPOSE_CHECKPOINTS`; Developers only; in-app notify default + `AUTOMATIONS_CHECKPOINT_NOTIFY_SMS/_EMAIL` opt-in):
- **Phase B (event sidecar):** runner executes under a SUPERVISED loop — stdout/stderr stream to `events.jsonl` per run (monotonic seq), live egress; endpoints `/automations/api/runs/<id>/events?after=N` + `/api/active` (+ internal manage actions `run_events`/`active` for CC).
- **Phase C (control plane):** `aihub.checkpoint("msg")` pauses the run at a human gate — state as `checkpoint_<id>.json` files in the workdir (NO new tables), run status flips running⇄waiting/aborting; Proceed resumes, Abort (or the abort endpoint) kills the child within ~2s → new honest outcome **aborted**; unanswered gates still hit the run timeout.
- **Phase A (Studio panel):** `cc-studio.js` + `#studio-panel` docked in the CC chat (Developer-only) — phase rail, contract card (manifest rendered), typewriter code reveal on save, dry-run verify theater, live run view (budget bar, log tail, egress chips), checkpoint gate + abort. Data via `studio_state.py` (per-session hint updated by the tool wrappers) + `routes/studio.py` proxies (CC-JWT → X-API-Key internal manage). **Sidebar change:** CC left panel (history+suggestions) hidden by default; New Chat/History/Memory/Schedules buttons moved to the chat header; History toggles it (localStorage pref).
- **Mission Control:** `GET /automations/` upgraded to a live board — active runs with event feed, budget bars, egress, checkpoint gates, abort, plus history/logs.
- Needs main-app + CC restarts (+ hard refresh for the CC frontend; cache-busted `?v=` bumps included).
