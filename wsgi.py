...
_default_log = os.path.join(_APP_ROOT, 'logs', 'app_log.txt')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.getenv('LOG_DIR', _default_log), encoding='utf-8'),
...
def start_server():
    ...
    logging.info(f"Starting production server on {host}:{port} with {threads} threads (channel_timeout={channel_timeout}s)")
    serve(
        app,
        host=host,
        port=port,
